...
def test_topography_object(plot=False):
    ...
    try:
        ...
        for (n, topo_type) in enumerate(topo_types):
            ...
            bowl_topo.append(topotools.Topography(bowl_path))
            hill_topo.append(topotools.Topography(hill_path))
            ...
            if plot:
                ...
                fig.suptitle(
                         'Bathymetry / topography, topo type = %s' % topo_type)
                plt.savefig('topotest%s.png' % (n + 2))

            print(n, topo_type)
            if n + 1 != len(topo_types):
                bowl_path = os.path.join(
                                    base_path, 'bowl.tt%s' % topo_types[n + 1])
                hill_path = os.path.join(base_path,
                                         'hill.tt%s' % topo_types[n + 1])
