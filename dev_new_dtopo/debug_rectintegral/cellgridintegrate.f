...
c              !integrate surface and get out of here
     ...
     &              topo(i0),im)
               write(45,451) xim,xip,yjm,yjp,m
 451           format(4e14.4, ' using grid m = ',i3)
               return
            else
               go to 222
            endif
...
c           ! integrate surface over intersection of grid and cell
     ...
     &              dytopo(mfid),mxtopo(mfid),mytopo(mfid),
     &              topo(i0),im)
	               write(45,452) xim,xip,yjm,yjp,1,m
 452               format(4e14.4, ' level ',i2,'  m = ',i3)
...
c                 ! get rid of coarser integral
     ...
     &              dytopo(mfid),mxtopo(mfid),mytopo(mfid),
     &              topo(i0),im)
	               write(45,452) xim,xip,yjm,yjp,2,mm
c                  -----------------------------------------------------
c                 ------------------------------------------------------------
...
c                       ! add back
     ...
     &                  dytopo(mfid),mxtopo(mfid),mytopo(mfid),
     &                                             topo(i0),im)
	                  write(45,452) xim,xip,yjm,yjp,3,mmm
...
c                          ! add back
     ...
     &                        mytopo(mfid),topo(i0),im)
                               write(45,452) xim,xip,yjm,yjp,4,m4

                                 do m5 = m4-1,1,-1
                                    mfidint = mtopoorder(m5)
c                                   ! check for 5th intersection
                                    write(45,452) xim,xip,yjm,yjp,5,m5
     ...
     &                                 ylowtopo(mfidint),
     &                                 yhitopo(mfidint))
                                    write(45,*) xlowtopo(mfidint),
     &                                 xhitopo(mfidint),
     &                                 ylowtopo(mfidint),
     &                                 yhitopo(mfidint)
                                    write(45,*) 'indicator, area: ',
     &                                 indicator, area
