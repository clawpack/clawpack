...
contains
    ...
    subroutine add_perturbation(maxmx,maxmy,meqn,mbc,mx,my,xlower,ylower,dx,dy,q,maux,aux)
        ...
        use geoclaw_module, only: layers,eta_init
        ...
        if (qinit_type > 0 .and. layers == 1) then
            do i=1-mbc,mx+mbc
                x = xlower + (i-0.5d0)*dx
                xim = x - 0.5d0*dx
                xip = x + 0.5d0*dx
