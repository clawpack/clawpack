...
class Storm(object):
    ...
    *TODO:*  Add description of unit handling
    ...
    :Attributes:
     ...
     - *max_wind_intensity_radius* (ndarray(:)) Radius of maximum record wind 
       intensity. Default units are kilometers.
    ...
    def __init__(self, path=None, file_format="ATCF", **kwargs):
        ...
        self.max_wind_intensity_radius = None
    ...
    def read_atcf(self, path, verbose=False):
        ...
        self.max_wind_intensity_radius = numpy.empty(num_lines)
        ...
        for (i, data) in enumerate(data_block):
            ...
            if self.central_pressure.min() == -1:
                warnings.warn('Some timesteps have missing central pressure. These will not be written'
                              ' out to geoclaw format.')
            print("t = ", self.t[i])                  
            print("wind_intensity = ", self.wind_intensity[i])
            print("wind_intensity_radius = ", self.wind_intensity_radius[i])
            print(" ")
